...
export const AuthProvider = ({ children }) => {
  ...
  const validateToken = async () => {
    try {
      const response = await fetch('https://explanation-bachelor-belkin-constraint.trycloudflare.com/api/admin/auth/validate', {
        method: 'GET',
        headers: {
  ...
  const login = async (username, password) => {
    try {
      const response = await fetch('https://explanation-bachelor-belkin-constraint.trycloudflare.com/api/admin/auth/login', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const logout = useCallback(async () => {
    try {
      if (token) {
        await fetch('https://explanation-bachelor-belkin-constraint.trycloudflare.com/api/admin/auth/logout', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
